...
class Wafer(object):
    ...
    def get_path(self):
        """根据晶圆类型返回对应的加工路径（含并行模块）"""
        if self.wafer_type == 1:
            # 类型1路径：LL→PM1→[PM2,PM3]（并行）→PM4→LL
            # 用列表嵌套表示并行模块（PM2和PM3二选一）
            return ["LL", "PM1", ["PM2", "PM3"], "PM4", "LL"]
        elif self.wafer_type == 2:
            # 类型2路径：LL→PM5→PM6→LL
            return ["LL", "PM5", "PM6", "LL"]
        else:
            return []
...
class Loadlock(object):
    ...
    def load(self, wafer):
        ...
        if wafer.wafer_type == 1 and wafer.process_step == 3:  # 类型1晶圆完成3个步骤
            pass  # 允许返回LL
        elif wafer.wafer_type == 2 and wafer.process_step == 2:  # 类型2晶圆完成2个步骤
            pass  # 允许返回LL
        else:
            self.fail = True
...
class Environment(object):
    ...
    def initialize(self):
        ...
        # 核心修改：调整PM允许处理的晶圆类型
        # PM1-3处理类型1，PM4处理类型1（类型1最后一步），PM5-6处理类型2
        pm_wafer_types = {
            "PM1": [1], "PM2": [1], "PM3": [1], "PM4": [1],  # 类型1专用（含并行模块和最后一步PM4）
            "PM5": [2], "PM6": [2]                           # 类型2专用
        }
    ...
    def get_mask(self):
        ...
        if all_pms == 0:
            for i, act in enumerate(self.actions):
                if act in [[0, 1],]:
                    mask[i] = True
                elif act in [[1, 2], [1, 3], [2, 4], [3, 4], [4, 0], [0, 5], [5, 6], [6, 0]]:
                    mask[i] = False
        ...
        # PM1 PM2 PM5
            ...
            for i, act in enumerate(self.actions):
                if act in [[1, 3],[2, 4],[5, 6]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [3, 4], [4, 0], [0, 5], [6, 0]]:
                    mask[i] = False


        # PM1 PM2 PM6
        elif pm1_wafer_count > 0 and pm2_wafer_count and pm6_wafer_count > 0 and (
                all_pms - pm1_wafer_count - pm2_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[1, 3],[2, 4],[0, 5],[6, 0]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [3, 4], [4, 0], [5, 6]]:
                    mask[i] = False

        # PM1 PM2 PM5 PM6
        elif pm1_wafer_count > 0 and pm2_wafer_count and pm5_wafer_count > 0 and pm6_wafer_count > 0 and (
                all_pms - pm1_wafer_count - pm2_wafer_count - pm5_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[1, 3]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [2, 4], [3, 4], [4, 0], [0, 5], [5, 6], [6, 0]]:
                    mask[i] = False

        # PM2 PM3 PM5 PM6
        elif pm2_wafer_count > 0 and pm3_wafer_count and pm5_wafer_count > 0 and pm6_wafer_count > 0 and (
                all_pms - pm2_wafer_count - pm3_wafer_count - pm5_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[2, 4]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [3, 4], [4, 0], [0, 5], [5, 6], [6, 0]]:
                    mask[i] = False

        # PM3 PM4 PM5 PM6
        elif pm3_wafer_count > 0 and pm4_wafer_count and pm5_wafer_count > 0 and pm6_wafer_count > 0 and (
                all_pms - pm3_wafer_count - pm4_wafer_count - pm5_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[0, 1], [6, 0]]:
                    mask[i] = True
                elif act in [[1, 2], [1, 3], [2, 4], [3, 4], [4, 0], [0, 5], [5, 6]]:
                    mask[i] = False

        # PM3 PM4 PM5
        elif pm3_wafer_count > 0 and pm4_wafer_count and pm5_wafer_count > 0 and (
        ...
        # PM1 PM3 PM4 PM5
        elif pm1_wafer_count and pm3_wafer_count > 0 and pm4_wafer_count and pm5_wafer_count > 0 and (
                all_pms - pm1_wafer_count - pm3_wafer_count - pm4_wafer_count - pm5_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[5, 6]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [4, 0], [0, 5], [6, 0]]:
                    mask[i] = False

        # PM1 PM3 PM4 PM6
        elif pm1_wafer_count and pm3_wafer_count > 0 and pm4_wafer_count and pm6_wafer_count > 0 and (
                all_pms - pm1_wafer_count - pm3_wafer_count - pm4_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[0, 5]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [4, 0], [5, 6], [6, 0]]:
                    mask[i] = False

        # PM1 PM3 PM4 PM5 PM6
        elif pm1_wafer_count and pm3_wafer_count > 0 and pm4_wafer_count and pm5_wafer_count > 0 and pm6_wafer_count and (
                all_pms - pm1_wafer_count - pm3_wafer_count - pm4_wafer_count - pm5_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[4, 0], [6, 0]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [0, 5], [5, 6]]:
                    mask[i] = False

        # PM1 PM3 PM5 PM6
        elif pm1_wafer_count and pm3_wafer_count > 0 and pm5_wafer_count > 0 and pm6_wafer_count and (
                all_pms - pm1_wafer_count - pm3_wafer_count - pm5_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[1, 2], [3, 4]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 3], [2, 4], [4, 0], [0, 5], [5, 6], [6, 0]]:
                    mask[i] = False

        # PM1 PM4 PM5 PM6
        elif pm1_wafer_count and pm4_wafer_count and pm5_wafer_count > 0 and pm6_wafer_count and (
                all_pms - pm1_wafer_count - pm4_wafer_count - pm5_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[1, 2]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 3], [2, 4], [3, 4], [4, 0], [0, 5], [5, 6], [6, 0]]:
        ...
        # PM2 PM4 PM5 PM6
        elif pm2_wafer_count and pm4_wafer_count and pm5_wafer_count > 0 and pm6_wafer_count and (
                all_pms - pm2_wafer_count - pm4_wafer_count - pm5_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[0, 1], [6, 0]]:
                    mask[i] = True
                elif act in [[1, 2], [1, 3], [2, 4], [3, 4], [4, 0], [0, 5], [5, 6]]:
        ...
        # PM2 PM4 PM5
        elif pm2_wafer_count and pm4_wafer_count and pm5_wafer_count > 0 and (
        ...
        # PM2 PM4 PM6
        elif pm2_wafer_count and pm4_wafer_count and pm6_wafer_count > 0 and (
                all_pms - pm2_wafer_count - pm4_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[4, 0],]:
                    mask[i] = True
                elif act in [[1, 2], [1, 3], [2, 4], [3, 4], [0, 5], [5, 6], [6, 0]]:
                    mask[i] = False

        # PM1 PM2 PM4 PM5 PM6
        elif pm1_wafer_count and pm2_wafer_count and pm4_wafer_count and pm5_wafer_count > 0 and pm6_wafer_count and (
                all_pms - pm1_wafer_count - pm2_wafer_count - pm4_wafer_count - pm5_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[6, 0], [4, 0]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [0, 5], [5, 6]]:
        ...
        # PM1 PM2 PM4 PM5
        elif pm1_wafer_count and pm2_wafer_count and pm4_wafer_count and pm5_wafer_count > 0 and (
        ...
        # PM1 PM2 PM4 PM6
        elif pm1_wafer_count and pm2_wafer_count and pm4_wafer_count and pm6_wafer_count > 0 and (
                all_pms - pm1_wafer_count - pm2_wafer_count - pm4_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[0, 5],]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [4, 0], [5, 6], [6, 0]]:
        ...
        #  PM4 PM6
            ...
            for i, act in enumerate(self.actions):
                if act in [[6, 0],]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [4, 0], [0, 5], [5, 6]]:
                    mask[i] = False

        #  PM4
        elif pm4_wafer_count > 0 and (all_pms - pm4_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[4, 0],]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [0, 5], [5, 6], [6, 0]]:
                    mask[i] = False
...
if __name__ == "__main__":
    ...
    #wafer_num = 10  总晶圆数量
    #actions = [0, 6, 1, 0, 7, 6, 2, 3, 8, 0, 7, 6, 5, 4, 1, 0, 8, 7, 6, 5, 2, 3, 8, 0, 7, 6, 5, 4, 1, 8, 7, 5, 3, 8, 5]  # 899
    actions = [0, 6, 1, 0, 7, 6, 2, 3, 8, 0, 7, 6, 5, 4, 1, 0, 8, 7, 6, 5, 2, 3, 0, 8, 7, 6, 5, 1, 4, 8, 7, 5, 3, 8, 5]  # 877
