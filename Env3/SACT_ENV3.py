...
单臂单组合设备调度环境
 LL->PM1->PM2//PM3->PM4->PM5->LL
 LL->PM1->PM2//PM3->PM4->PM6->LL
...
class Wafer(object):
    ...
    def get_path(self):
        ...
        核心修改：两种类型晶圆仅在最后一步不同
        类型1: LL->PM1->[PM2,PM3]->PM4->PM5->LL
        类型2: LL->PM1->[PM2,PM3]->PM4->PM6->LL
        """
        if self.wafer_type == 1:
            # 类型1路径：共享路径+PM5+LL
            return ["LL", "PM1", ["PM2", "PM3"], "PM4", "PM5", "LL"]
        elif self.wafer_type == 2:
            # 类型2路径：共享路径+PM6+LL
            return ["LL", "PM1", ["PM2", "PM3"], "PM4", "PM6", "LL"]
        else:
            return []
...
class Environment(object):
    ...
    def initialize(self):
        ...
        # 核心修改：调整PM允许处理的晶圆类型
        # 共享路径模块(PM1-PM4)允许处理两种类型，PM5仅处理类型1，PM6仅处理类型2
        pm_wafer_types = {
            "PM1": [1, 2],  # 两种类型都可以处理
            "PM2": [1, 2],  # 两种类型都可以处理
            "PM3": [1, 2],  # 两种类型都可以处理
    ...
    def execute_action(self, action):
        ...
        if source_module == "LL" and target_module == "PM1":
            ...
            print(f"预期下次类型：{self.next_target_type}")

            # 1. 确定需要检查的类型（必须是next_target_type）
            required_type = self.next_target_type
            ...
            self.next_target_type = 3 - actual_type  # 无论本次取什么，下次必为相反类型
    ...
    def get_mask(self):
        ...
        if all_pms == 0:
            for i, act in enumerate(self.actions):
                if act in [[0, 1]]:
                    mask[i] = True
                elif act in [[1, 2], [1, 3], [2, 4], [3, 4], [4, 5], [5, 0], [4, 6], [6, 0]]:
                    mask[i] = False
        ...
        #  PM1存在晶圆，其他PM不存在时，只允许[1,2]
            ...
            for i, act in enumerate(self.actions):
                if act in [[1, 2]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 3], [2, 4], [3, 4], [4, 5], [5, 0], [4, 6], [6, 0]]:
                    mask[i] = False
        ...
        #  PM2
            ...
            for i, act in enumerate(self.actions):
                if act in [[0, 1],[2, 4],]:
                    mask[i] = True
                elif act in [[1, 2], [1, 3], [3, 4], [4, 5], [5, 0], [4, 6], [6, 0]]:
                    mask[i] = False

        #  PM1和PM2同时存在，其他PM不存在时，只允许[0,1]
        elif pm1_wafer_count > 0 and pm2_wafer_count > 0 and (all_pms - pm1_wafer_count - pm2_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[1, 3],[2, 4],]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [3, 4], [4, 5], [5, 0], [4, 6], [6, 0]]:
                    mask[i] = False
        ...
        # PM2 PM3
            ...
            for i, act in enumerate(self.actions):
                if act in [[2, 4],[3, 4],]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [4, 5], [5, 0], [4, 6], [6, 0]]:
                    mask[i] = False


        # PM3 PM4
        elif pm3_wafer_count > 0 and pm4_wafer_count > 0 and (
                all_pms - pm3_wafer_count - pm3_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[0, 1], [4, 5]]:
                    mask[i] = True
                elif act in [[1, 2], [1, 3], [2, 4], [3, 4], [5, 0], [4, 6], [6, 0]]:
                    mask[i] = False
        ...
        # PM1 PM3 PM4
            ...
            for i, act in enumerate(self.actions):
                if act in [[1, 2],[4, 5]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [5, 0], [4, 6], [6, 0]]:
                    mask[i] = False
        ...
        # PM1 PM3 PM5
            ...
            for i, act in enumerate(self.actions):
                if act in [[1, 2], [3, 4], [5, 0],]:
                    mask[i] = True
                elif act in [[0, 1], [1, 3], [2, 4], [4, 5], [4, 6], [6, 0]]:
                    mask[i] = False
        ...
        # PM1 PM4 PM5
            ...
            for i, act in enumerate(self.actions):
                if act in [[1, 3],[1, 2],[5, 0],]:
                    mask[i] = True
                elif act in [[0, 1], [2, 4], [3, 4], [4, 5], [5, 0], [4, 6], [6, 0]]:
                    mask[i] = False
        ...
        # PM2 PM4 PM5
            ...
            for i, act in enumerate(self.actions):
                if act in [[0, 1],[4, 5],[5, 0],]:
                    mask[i] = True
                elif act in [[1, 2], [1, 3], [2, 4], [3, 4], [4, 6], [6, 0]]:
                    mask[i] = False
        ...
        # PM1 PM2 PM4 PM5
            ...
            for i, act in enumerate(self.actions):
                if act in [[5, 0],[4, 6]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [4, 5], [6, 0]]:
                    mask[i] = False

        # PM1 PM2 PM5 PM6
        elif pm1_wafer_count > 0 and pm2_wafer_count > 0 and pm5_wafer_count > 0 and pm6_wafer_count > 0 and (
                all_pms - pm1_wafer_count - pm2_wafer_count - pm5_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[2, 4],[5, 0],[6, 0]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [3, 4], [4, 5], [4, 6], [6, 0]]:
                    mask[i] = False
        ...
        # PM1 PM2 PM6
            ...
            for i, act in enumerate(self.actions):
                if act in [[2, 4],[1, 3],[6, 0]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [3, 4], [4, 5], [5, 0], [4, 6],]:
                    mask[i] = False
        ...
        # PM2 PM3 PM6
            ...
            for i, act in enumerate(self.actions):
                if act in [[2, 4], [3, 4],[6, 0]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [4, 5], [5, 0], [4, 6],]:
                    mask[i] = False
        ...
        # PM3 PM4 PM6
            ...
            for i, act in enumerate(self.actions):
                if act in [[0, 1], [6, 0]]:
                    mask[i] = True
                elif act in [[1, 2], [1, 3], [2, 4], [3, 4], [4, 5], [5, 0], [4, 6]]:
                    mask[i] = False

        # PM1 PM3 PM4 PM6
        elif pm1_wafer_count > 0 and pm3_wafer_count > 0 and pm4_wafer_count > 0 and pm6_wafer_count > 0 and (
                all_pms - pm1_wafer_count - pm3_wafer_count - pm4_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[6, 0],]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [4, 5], [5, 0], [4, 6]]:
                    mask[i] = False
        ...
        # PM1 PM3 PM5
            ...
            for i, act in enumerate(self.actions):
                if act in [[3, 4],]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [4, 5], [5, 0], [4, 6], [6, 0]]:
                    mask[i] = False
        ...
        # PM1 PM2 PM4 PM6
        elif pm1_wafer_count > 0 and pm2_wafer_count > 0 and pm4_wafer_count > 0 and pm6_wafer_count > 0 and (
        ...
        # PM1 PM2 PM5
            ...
            for i, act in enumerate(self.actions):
                if act in [[2, 4],]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [3, 4], [4, 5], [5, 0], [4, 6], [6, 0]]:
                    mask[i] = False
        ...
        # PM3 PM5
        elif pm3_wafer_count > 0 and pm5_wafer_count > 0 and (
                all_pms - pm3_wafer_count - pm5_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[3, 4]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [4, 5], [5, 0], [4, 6], [6, 0]]:
        ...
        # PM4 PM5
            ...
            for i, act in enumerate(self.actions):
                if act in [[4, 6]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [4, 5], [5, 0], [6, 0]]:
                    mask[i] = False

        # PM5 PM6
        elif pm5_wafer_count > 0 and pm6_wafer_count > 0 and (
                all_pms - pm5_wafer_count - pm6_wafer_count) == 0:
            for i, act in enumerate(self.actions):
                if act in [[5, 0]]:
                    mask[i] = True
                elif act in [[0, 1], [1, 2], [1, 3], [2, 4], [3, 4], [4, 5], [4, 6], [6, 0]]:
                    mask[i] = False
...
if __name__ == "__main__":
    ...
    process_residency_time_dict = {"LL": [0, 0], "PM1": [90, 20], "PM2": [120, 20], "PM3": [120, 20], "PM4": [100, 20],
                                   "PM5": [110, 20], "PM6": [100, 20]}
    ...
    process_time_dict = {"LL": 0, "PM1": 90, "PM2": 120, "PM3": 120, "PM4": 100, "PM5": 110, "PM6": 100}
    process_time_list = [0, 90, 120, 120, 100, 110, 100]
    ...
    # 动作空间
    actions = [
        [0, 1], [1, 2], [1, 3], [2, 4], [3, 4],  # 0 1 2 3 4
        [4, 5], [5, 0],  # 类型1路径: LL→PM1→PM2//PM3→PM4→PM5→LL   5 6
        [4, 6], [6, 0],  # 类型2路径: LL→PM1→PM2//PM3→PM4→PM6→LL   7 8
    ]
    ...
    # 适用于10个晶圆的测试动作序列
    #1833
    #actions = [0, 1, 0, 2, 3, 0, 5, 4, 1, 7, 6, 0, 3, 8, 1, 5, 0, 3, 2, 6, 0, 7, 4, 1, 0, 8, 5, 2, 3, 0, 6, 7, 4, 1, 0, 8, 7, 2, 3, 0, 8, 7, 4, 1, 8, 5, 3, 6, 5, 6]
    #1739
    #actions = [0, 1, 0, 2, 3, 0, 5, 4, 1, 0, 7, 6, 2, 3, 0, 8, 5, 4, 1, 0, 7, 6, 2, 3, 0, 8, 5, 4, 1, 0, 7, 6, 2, 3, 0, 8, 7, 4, 1, 0, 8, 7, 2, 3, 8, 5, 4, 6, 5, 6]


    #改为类型1和2严格交替从LL加载到PM1上：1724
    actions = [0, 1, 0, 2, 3, 0, 5, 4, 1, 0, 7, 6, 2, 3, 0, 8, 5, 4, 1, 0, 7, 6, 2, 3, 0, 8, 5, 4, 1, 0, 7, 6, 2, 3, 0, 8, 5, 4, 1, 0, 7, 6, 2, 3, 8, 5, 4, 7, 6, 8]
